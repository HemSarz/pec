...
# AAG = Admin Agent Group
# QRO = Quota Request Operator
...
function ateaVariables {
    $global:ateaSpnId = "1069b51f-753f-42de-bf97-df237e2dcd70"
...
# Function to check if SPN exists or create if not
function CreateOrRetrieveSPN {
    Write-Host "Checking if ATEA Service Principal already exists." -ForegroundColor Cyan
    $global:spnObjectId = az ad sp show --id $ateaSpnId --query id -o tsv
    
    if ($global:spnObjectId) {
        Write-Host "Service Principal already exists with Object ID: $global:spnObjectId" -ForegroundColor Green
    }
    else {
        Write-Host "Service Principal not found. Creating ATEA Service Principal." -ForegroundColor Cyan
        # Create the Service Principal
        $global:spnObjectId = az ad sp create --id $ateaSpnId --query id -o tsv
        
        if (-not $global:spnObjectId) {
            Write-Host "Failed to create SPN or retrieve Object ID. Please verify the SPN Client ID and try again." -ForegroundColor Red
            exit 1
        }
        else {
            Write-Host "Service Principal created and Object ID retrieved successfully: $global:spnObjectId" -ForegroundColor Green
        }
    }
}
...
function AssignSRCRole {
    ...
    foreach ($scope in $scopes) {
        ...
        az role assignment create `
            --role "Support Request Contributor" `
            --assignee-object-id $adminAgentsGroupId `
...
function VerifySPNRole {
    Write-Host "Retrieve the ATEA SPN roles in the customer tenant." -ForegroundColor Magenta
    az role assignment list --assignee $ateaSpnId --all -o table   
}

# Retrieve & verify role assignment
function VerifyAdminAgentsRole {
    Write-Host "Retrieve the ATEA SPN roles in the customer tenant." -ForegroundColor Magenta
